-- Fix: digest() requires pgcrypto on the search_path (Supabase keeps it in schema "extensions").
-- Also use an explicit jsonb loop alias and accept numeric durationMs from JSON.
...
BEGIN
  ...
  vk := encode(digest(convert_to('nv_analytics_v1:' || trim(p_visitor_id), 'UTF8'), 'sha256'), 'hex');

  FOR ev IN
    SELECT elem FROM jsonb_array_elements(coalesce(p_events, '[]'::jsonb)) AS t(elem)
  LOOP
    EXIT WHEN n >= 50;
    ...
    dms := NULL;
    IF ev ? 'durationMs' AND ev->>'durationMs' IS NOT NULL THEN
      IF jsonb_typeof(ev->'durationMs') = 'number' THEN
        dms := (ev->'durationMs')::text::int;
      ELSIF ev->>'durationMs' ~ '^-?[0-9]+$' THEN
